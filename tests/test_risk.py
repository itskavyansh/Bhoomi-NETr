...
def test_sensor_health_hardware_fault_override():
    ...
    assert health["sensor_health"]["mpu6050_status"] == "fault"
    assert health["sensor_health"]["hc_sr04_status"] == "ok"
    assert health["confidence_warning"] == "LOW SENSOR CONFIDENCE: MPU6050 hardware fault reported"
...
def test_sensor_health_hardware_ok_status():
    ...
    assert health["sensor_health"]["mpu6050"] == "GOOD"
    assert health["sensor_health"]["hcsr04"] == "GOOD"
    assert health["sensor_health"]["mpu6050_status"] == "ok"
    assert health["sensor_health"]["hc_sr04_status"] == "ok"
    assert health["confidence_warning"] is None
